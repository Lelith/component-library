...
class ConfigurationTree extends React.Component {
  ...
  getRowStyle() {
    ...
    if (pivotOffset && nodes.length < columnCount) {
      ...
      const visibleWidth = this.getVisibleWidth(); // columnWidth * Math.min(columnCount, nodes.length - 1);
  ...
  renderChildNodes() {
    ...
    const parentPivot = this.getPivotOffset();
    const childPivot = parentPivot - (visibleWidth / 2) + (activeNode.index * columnWidth) + (columnWidth / 2);
    const pivotOffset = childPivot;
    return (
      <ConfigurationTree nodes={activeNode.node.nodes} pivotOffset={pivotOffset} />
    );
  }
