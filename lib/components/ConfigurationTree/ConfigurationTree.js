...
class ConfigurationTree extends React.Component {
  ...
  getRowOffset() {
    ...
    if (pivotOffset && nodes.length < columnCount) {
      ...
      const maxRowOffset = (columnWidth * columnCount) - (visibleWidth + additionalOffset);
      const rowOffset = Math.max(pivotOffset - (visibleWidth / 2), 0);
      return {
  ...
  getScrollColumns(direction) {
    let scrollColumns;
    if (direction === 'left') {
      scrollColumns = this.state.scrollIndex;
    } else if (direction === 'right') {
      scrollColumns = this.props.nodes.length - this.getColumnCount() - this.state.scrollIndex;
    }
  ...
  renderArrow(direction) {
    ...
    return (
      <div
        ...
        style={this.getArrowStyle(direction)}
...
ConfigurationTree.propTypes = {
  ...
  pivotOffset: PropTypes.number,
  parentIsLastActive: PropTypes.boolean,
  parentVisible: PropTypes.boolean,
};
