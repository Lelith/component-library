...
class ConfigurationTree extends React.Component {
  ...
  renderNode(node, index) {
    ...
    const nodeOffset = rowOffset + (index * columnWidth);
    const nodeLeftOffset = nodeOffset + (columnWidth / 4);
    const nodeRightOffset = nodeOffset + ((columnWidth * 3) / 4);
    const activeNode = this.getActiveNode();
    const activeNodeOffset = rowOffset + (activeNode.index * columnWidth) + (columnWidth / 2);
  ...
  renderChildNodes() {
    ...
    const childPivot = parentPivot
      ? parentPivot - (visibleWidth / 2) + (activeNode.visibleIndex * columnWidth) + (columnWidth / 2)
      : (activeNode.visibleIndex * columnWidth) + (columnWidth / 2);
    return (
      <ConfigurationTree nodes={activeNode.node.nodes} pivotOffset={childPivot} parentVisible={activeNode.isVisible} />
    );
  }

  render() {
    return (
      <div className={classNames('ct-wrapper', { 'ct-wrapper--hidden': !this.props.parentVisible && this.props.parentVisible != null })}>
        <div className="ct">
