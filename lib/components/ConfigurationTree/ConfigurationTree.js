...
class ConfigurationTree extends React.Component {
  ...
  constructor(props) {
    ...
    this.handleChildTransitionExit = this.handleChildTransitionExit.bind(this);
  }
  ...
  renderNode(node, index) {
    ...
    const showSideLines = node.type !== 'Add' && normalNodes.length > 1;
    const showUpLine = node.type !== 'Add' || this.props.nodes.length <= 1;
    ...
    if (activeNode && normalNodes.length > 1) {
